...
Ftoexpand = F;
try
    Ftoexpand = Ftoexpand - Ftoexpand('update');
end
try
    Ftoexpand = Ftoexpand - Ftoexpand('output');
end
try
    Ftoexpand = Ftoexpand - Ftoexpand('bounds');
end
for i = 1:length(Ftoexpand)
    ctype = sub_classify_constraint(Ftoexpand(i));
    switch ctype
        case 'implies'
            Ftotal = Ftotal + sub_optimize_implies(Ftoexpand(i), Flin);
            sub_update_bounds(F+Ftotal);
            
        case 'iff'
            Ftotal = Ftotal + sub_optimize_iff(Ftoexpand(i), Flin);
            sub_update_bounds(F+Ftotal);
            
        case 'ife'
            Ftotal = Ftotal + sub_optimize_ifthenelse(Ftoexpand(i), Flin);
            sub_update_bounds(F+Ftotal);
    
        otherwise
            Fexp = expandmodel(Ftoexpand(i), [], H3_SDPSETTINGS);
            Ftotal = Ftotal + Fexp;
            sub_update_bounds(F + Ftotal);
    end
end

% finally expand additional binaries
Fexp = expandmodel(Fadditional_d, [], H3_SDPSETTINGS);
Ftotal = Ftotal + Fexp;
sub_update_bounds(F+Ftotal);
...
% % special case: all bounds are equal
% if isa(Mx, 'double') && isa(mx, 'double') && ...
%         isa(My, 'double') && isa(my, 'double') && ...
%         norm(Mx-mx)<1e-8 && norm(My-my)<1e-8 && norm(Mx-My)<1e-8
%     F = set(mx <= z <= my, 'bounds');
%     F = expandmodel(F, [], H3_SDPSETTINGS);
%     return
% end
